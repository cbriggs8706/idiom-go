...
export default function TestingDictionary({ data }: DictionaryProps) {
	...
	const grouped = useMemo(() => {
		const byLetter: Record<string, Word[]> = {}
	...
	const groupedByLessonRange = useMemo(() => {
		const byRange: Record<string, Word[]> = {}
				...
				<div className="flex items-center justify-between w-full p-1">
					<div className="flex gap-2 items-center w-full">
						{entry.hebAudio && (
							<button
								...
								onClick={(e) => {
									...
									playAudio(entry.id ?? 0, entry.hebAudio ?? undefined)
								}}
						...
						<div className="ml-auto flex items-center gap-6">
							{parseGenderPerson(entry.genderPerson ?? undefined)}
				...
				{sortMode === 'alphabetical' ? (
					...
					<>
						{Object.entries(groupedByLessonRange).map(([range, entries]) => {
							...
							const byLesson: Record<number, Word[]> = {}
							for (const entry of entries) {
